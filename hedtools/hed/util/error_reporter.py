...
def push_error_context(context_type, context, increment_depth_after=True, column_context=None):
    """
    ...
    error_context_stack.append(f"{default_tabbing}{context_message}")
    if increment_depth_after:
        current_tab_depth += 1


def pop_error_context(decrement_depth_after=True):
    """
...
def reset_error_context():
    ...
    global current_tab_depth
    current_tab_depth = 0
    global error_context_stack
    error_context_stack = []
    global last_used_error_context
    last_used_error_context = []


def _add_context_to_errors(error_object):
    """
    ...
    if last_used_error_context == error_context_stack:
        return [error_object]

    error_context = []
    for i, context in enumerate(error_context_stack):
        if len(last_used_error_context) > i:
            last_drawn = last_used_error_context[i]
    ...
    last_used_error_context = error_context_stack.copy()

    return error_context + [error_object]


def format_val_error(error_type, hed_string='', tag='', tag_prefix='', previous_tag='',
    ...
    error_object_list = _add_context_to_errors(error_object)
...
def format_val_warning(warning_type, tag='', default_unit='', tag_prefix=''):
    ...
    warning_object_list = _add_context_to_errors(warning_object)
    return warning_object_list


def reformat_schema_error(error, hed_string, offset):
    """
    ...
    reformatted_error = format_schema_error(error_type, hed_string, error_index + offset, error_index_end + offset, expected_parent_tag)
...
def format_schema_warning(error_type, hed_tag, hed_desc=None, error_index=0, problem_char=None):
