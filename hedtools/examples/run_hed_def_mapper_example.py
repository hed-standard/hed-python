...
if __name__ == '__main__':
    ...
    hed_dict = HedSchema(local_hed_file)
    ...
    validation_issues = input_file.validate_file_sidecars(hed_schema=hed_dict)
    if validation_issues:
        print("There should be no errors with the sidecar.  This will likely cause other errors.")
        for issue in validation_issues:
            print(f"{issue['code']}: {issue['message']}")
    validator = HedValidator(input_file, hed_schema=hed_dict)
