...
from hed.tools import get_row_hash
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.stern_test1_path = os.path.join(base_dir, "sternberg_test_events.tsv")
        cls.stern_test2_path = os.path.join(base_dir, "sternberg_with_quotes_events.tsv")
        cls.attention_shift_path = os.path.join(base_dir, "auditory_visual_shift_events.tsv")
        cls.key_cols = ['type']
        cls.target_cols = ['event_type', 'task_role', 'letter']
        cls.count_col = ['counts']

    def test_constructor(self):
        t_map = KeyDict(self.key_cols)
        self.assertIsInstance(t_map, KeyDict)
        df = t_map.col_map
        ...
        try:
            ...
            self.fail(f'KeyDict threw the wrong exception {ex} when no columns')
        else:
            self.fail('KeyDict should have thrown a HedFileError exception when no columns')

        emap1 = KeyDict(['a', 'b', 'c'], name='baloney')
        self.assertIsInstance(emap1, KeyDict, "KeyDict: multiple columns are okay")
    ...
    def test_make_template(self):
        ...
        t_map2 = KeyDict(['event_type', 'type'])
    ...
    def test_make_template_key_overlap(self):
        ...
        try:
            ...
            self.fail('KeyDict should have thrown a HedFileError exception when key overlap but threw none')

    def test_update_key(self):
        t_map = KeyDict(self.target_cols)
    ...
    def test_print(self):
        ...
        t_map = KeyDict(self.key_cols+self.target_cols)
        t_map.update(self.stern_map_path)
        t_map.update(self.stern_map_path)
