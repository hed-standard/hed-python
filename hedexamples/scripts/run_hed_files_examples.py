...
if __name__ == '__main__':
    ...
    print(get_printable_issue_string(validation_issues, title='[Example 1b] ValidTwoColumnHED7_1_1 should have no issues with local version 7.1.1'))
    ...
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 1d] Valid TSV with required tags in separate columns'))
    ...
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3a] Multiple sheet xlsl has LKT Events sheet with no issues'))
    ...
    input_file = HedInput(multiple_sheet_xlsx_file, tag_columns=[4],
                          ...
                          worksheet_name='LKT Events')
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3b] LKT Events sheet probably has with no issues with the default schema'))

    # Example 3c: XLSX file with multiple sheets - assumes first sheet by default
    input_file = HedInput(multiple_sheet_xlsx_file, tag_columns=[4],
                          column_prefix_dictionary=prefixed_needed_tag_columns)
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3c] Multiple sheet xlsl has first sheet with no issues'))
    ...
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3d] Some PVT worksheet issues that are due to removal of extensionAllowed in places in 7.1.1'))
    ...
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3e] DAS Events worksheet has multiple issues with 7.1.1'))
    ...
    validation_issues = input_file.validate_file(hed_validator_local)
    print(get_printable_issue_string(validation_issues, title='[Example 3f] DAS worse now because of duplicate Label and Description specifications'))
