...
from hed.tools import EventValueSummary, FileDictionary
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.wh_events_path = \
            os.path.realpath(os.path.join(cls.bids_base_dir, 'sub-002/eeg/sub-002_task-FacePerception_run-1_events.tsv'))

    def test_constructor(self):
        dict1 = EventValueSummary()
        self.assertIsInstance(dict1, EventValueSummary, "EventValueSummary constructor is allowed to have no arguments")
        self.assertFalse(dict1.value_info)

        dict2 = EventValueSummary(value_cols=['a', 'b', 'c'], name='baloney')
        self.assertIsInstance(dict2, EventValueSummary, "EventValueSummary: multiple values are okay in constructor")
        self.assertEqual(len(dict2.value_info.keys()), 3, "EventValueSummary should have keys for each value column")
    ...
    def test_get_number_unique_values(self):
        ...
        self.assertEqual(len(dict1.value_info.keys()), 0,
                         "EventValueSummary value_info should be empty if no value columns")
        self.assertEqual(len(dict1.categorical_info.keys()), len(wh_df.columns),
                         "EventValueSummary categorical_info should have all the columns if no restrictions")
    ...
    def test_print(self):
        ...
        self.assertEqual(len(t_map.categorical_info.keys()), len(df.columns),
                         "EventValueSummary should have all columns as categorical if no value or skip are given")
    ...
    def test_update(self):
        ...
        dict2 = EventValueSummary(value_cols=['letter'], skip_cols=['event_type'])
        dict2.update(stern_df)
        self.assertEqual(len(dict2.value_info.keys()), 1,
                         "EventValueSummary value_info should have letter value column")
        self.assertEqual(dict2.value_info['letter'], len(stern_df),
                         "EventValueSummary value counts should length of column")
        self.assertEqual(len(dict2.skip_cols), 1, "EventValueSummary should have one skip column")
        self.assertEqual(len(dict2.categorical_info.keys()), len(stern_df.columns) - 2,
                         "EventValueSummary categorical_info should have all the columns except value and skip columns")
        dict2.update(stern_df)
        self.assertEqual(len(dict2.value_info.keys()), 1,
                         "EventValueSummary value_info should have letter value column")
        self.assertEqual(dict2.value_info['letter'], 2*len(stern_df),
                         "EventValueSummary value counts should update by column length each time update is called")

    def test_update_dict(self):
        dict1 = EventValueSummary()
        dict2 = EventValueSummary()
    ...
    def test_update_dict_with_value_cols(self):
        ...
        dict1 = EventValueSummary(value_cols=['latency'])
        dict1.update(stern_df_test3)
        dict2 = EventValueSummary(value_cols=['latency'])
    ...
    def test_update_dict_bad_skip_col(self):
        ...
        dict1 = EventValueSummary(skip_cols=['latency'])
    ...
    def test_make_combined_dicts(self):
        ...
        file_dict = FileDictionary(files_bids)
        dicts_all1, dicts1 = EventValueSummary.make_combined_dicts(file_dict)
        self.assertTrue(isinstance(dicts_all1, EventValueSummary),
                        "make_combined_dicts should return a EventValueSummary")
        ...
        dicts_all2, dicts2 = EventValueSummary.make_combined_dicts(file_dict, skip_cols=["onset", "duration", "sample"])
        self.assertTrue(isinstance(dicts_all2, EventValueSummary),
                        "make_combined_dicts should return a EventValueSummary")
