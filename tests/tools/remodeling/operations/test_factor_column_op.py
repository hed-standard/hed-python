...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.factored = [[0.0776, 0.5083, 'go', 'n/a', 0.565, 'correct', 'right', 'female', 0, 0],
                        [5.5774, 0.5083, 'unsuccessful_stop', 0.2, 0.49, 'correct', 'right', 'female', 0, 1],
                        [9.5856, 0.5084, 'go', 'n/a', 0.45, 'correct', 'right', 'female', 0, 0],
                        [13.5939, 0.5083, 'successful_stop', 0.2, 'n/a', 'n/a', 'n/a', 'female', 1, 0],
        ...
        cls.default_factor_columns = ["trial_type.successful_stop", "trial_type.unsuccessful_stop"]

    def setUp(self):
        self.base_parameters = {
            "column_name": "trial_type",
            "factor_values": ["successful_stop", "unsuccessful_stop"],
    ...
    def test_valid_factors_extras_ignore(self):
        ...
        df = pd.DataFrame(self.sample_data, columns=self.sample_columns)
        df_check = pd.DataFrame(self.factored, columns=self.sample_columns + self.base_parameters['factor_names'])
        self.base_parameters["factor_values"] = ["successful_stop", "unsuccessful_stop", "face"]
