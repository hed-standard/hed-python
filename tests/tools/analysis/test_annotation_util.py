...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sidecar2c = {
            "a": {"c": {"c1": "blech3", "c2": "blech3a"}, "d": "blech4", "e": "blech5"},
            "b": {
                "HED": {"b2": "Purple,Description/Bad purple", "b3": "Red"},
        ...
        cls.sidecar3 = {
            ...
            "b": {"HED": {"b4": "Purple", "b5": "Red"}, "Levels": {"b4": "Its purple.", "b5": "Its red."}},
            "c": {"HED": {"b6": "Purple", "b7": "Red"}, "Levels": {"b6": "Its purple.", "b7": "Its red."}},
        }
    ...
    def test_df_to_hed_nas(self):
        data = [["event_code", "baloney", "n/a", "n/a"], ["event_code", "sausage", "this is sausage", "n/a"]]
    ...
    def test_hed_to_df_with_definitions(self):
        ...
        self.assertEqual(
            description, df1.loc[11, "description"], "hed_to_df should have right description when in parentheses"
        )
    ...
    def test_merge_hed_dict_full(self):
        ...
        event_files = io_util.get_file_list(
            self.bids_root_path, extensions=[".tsv"], name_suffix="events", exclude_dirs=exclude_dirs
        )
    ...
    def test_flatten_cat_col(self):
        ...
        self.assertEqual(descriptions2[0], "Bad purple", "_flatten_cat_col should use the Description tag if available")

    def test_flatten_cat_col_only_description(self):
        keys, values, descriptions, tags = annotation_util._flatten_cat_col(
            "event_type", {"HED": {"code1": "Description/Code 1 here."}}
        )
    ...
    def test_get_value_entry(self):
        ...
        self.assertNotIn(
            "Description", dict3, "_get_value_entry should not have a Description entry when tags but no description"
        )
        ...
        self.assertEqual(
            dict6["HED"],
            "Red,Blue, Description/This is a test",
            "_get_value_entry should correct value when Description used for HED tags",
        )
        self.assertIn("Description", dict6, "_get_value_entry should have a Description entry when Description used")
    ...
    def test_to_factor(self):
        ...
        self.assertEqual(len(series2), len(factor2))
        self.assertEqual(sum(factor2), 1)
        data = {"Name": ["Alice", "", "n/a", 1.0], "Age": [25, np.nan, 35, 0]}  # Contains a space
        df = DataFrame(data)
