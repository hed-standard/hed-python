...
class Test(unittest.TestCase):
    ...
    def test_merge_tag_dicts(self):
        ...
        for k in range(6):
            counts1.update_tag_counts(
                HedString(self.input_df.iloc[k]["HED_assembled"], self.hed_schema), file_name="Base_name1"
            )
            counts2.update_tag_counts(
                HedString(self.input_df.iloc[k]["HED_assembled"], self.hed_schema), file_name="Base_name2"
            )
    ...
    def test_hed_tag_count(self):
        ...
        counts1.update_tag_counts(HedString(self.input_df.iloc[0]["HED_assembled"], self.hed_schema), file_name=name)
