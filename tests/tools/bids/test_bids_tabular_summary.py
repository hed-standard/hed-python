...
class Test(unittest.TestCase):
    ...
    def test_constructor(self):
        dict1 = BidsTabularSummary()
        self.assertIsInstance(dict1, BidsTabularSummary,
                              "BidsTabularSummary constructor is allowed to have no arguments")
        self.assertFalse(dict1.value_info)

        dict2 = BidsTabularSummary(value_cols=['a', 'b', 'c'], name='baloney')
        self.assertIsInstance(dict2, BidsTabularSummary, "BidsTabularSummary: multiple values are okay in constructor")
    ...
    def test_update(self):
        ...
        dict2 = BidsTabularSummary(value_cols=['letter'], skip_cols=['event_type'])
    ...
    def test_update_dict_with_value_cols(self):
        ...
        dict1 = BidsTabularSummary(value_cols=['latency'])
        dict1.update(stern_df_test3)
        dict2 = BidsTabularSummary(value_cols=['latency'])
    ...
    def test_update_dict_bad_skip_col(self):
        ...
        dict1 = BidsTabularSummary(skip_cols=['latency'])
    ...
    def test_get_columns_info(self):
        ...
        col_info = BidsTabularSummary.get_columns_info(df)
    ...
    def test_get_columns_info_skip_columns(self):
        df = get_new_dataframe(self.stern_test2_path)
        col_info = BidsTabularSummary.get_columns_info(df, ['latency'])
        self.assertIsInstance(col_info, dict, "get_columns_info should return a dictionary")
        ...
        col_info = BidsTabularSummary.get_columns_info(df, list(df.columns.values))
        self.assertIsInstance(col_info, dict, "get_columns_info should return a dictionary")
    ...
    def test_make_combined_dicts(self):
        ...
        dicts_all1, dicts1 = BidsTabularSummary.make_combined_dicts(file_dict)
        self.assertTrue(isinstance(dicts_all1, BidsTabularSummary),
        ...
        dicts_all2, dicts2 = BidsTabularSummary.make_combined_dicts(file_dict,
                                                                    skip_cols=["onset", "duration", "sample"])
        self.assertTrue(isinstance(dicts_all2, BidsTabularSummary),
                        "make_combined_dicts should return a BidsTabularSummary")
