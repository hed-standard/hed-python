...
class Test(unittest.TestCase):
    ...
    def test_bids_tsv_dictionary_constructor_valid(self):
        ...
        dict1 = BidsTsvDictionary(file_list, entities=('sub', 'run'))
        self.assertEqual(6, len(dict1.key_list), "BidsTsvFileDictionary should have correct number of entries when key okay")

    def test_bids_tsv_dictionary_constructor_invalid(self):
        file_list = get_file_list(self.bids_base_dir, name_suffix="_events",
                                  extensions=['.tsv'], exclude_dirs=['stimuli'])
        try:
            dict1 = BidsTsvDictionary(file_list, entities=('sub'))
        except HedFileError:
            pass
        except Exception:
