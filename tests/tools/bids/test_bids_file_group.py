...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.file_paths = io_util.get_file_list(
            root_path, extensions=[".tsv", ".json"], exclude_dirs=exclude_dirs, name_suffix=["_events"]
        )
        cls.root_path = root_path
        cls.exclude_dirs = exclude_dirs
        cls.event_path = os.path.realpath(
            os.path.join(os.path.dirname(__file__), "../../data/bids_tests/eeg_ds003645s_hed/sub-002", file_name)
        )
    ...
    def test_summarize(self):
        ...
        self.assertEqual(
            len(info2.categorical_info),
            len(info.categorical_info) - 2,
