...
class TestSchemaFormatRoundtrip(unittest.TestCase):
    ...
    def _test_format_roundtrip(self, schema, schema_name, save_merged=True):
        ...
        self.assertEqual(schema, schema_from_xml,
                        f"XML roundtrip failed for {schema_name} (save_merged={save_merged})")
        self.assertEqual(schema, schema_from_mediawiki,
                        f"MediaWiki roundtrip failed for {schema_name} (save_merged={save_merged})")
        self.assertEqual(schema, schema_from_tsv,
                        f"TSV roundtrip failed for {schema_name} (save_merged={save_merged})")
        self.assertEqual(schema, schema_from_json,
                        f"JSON roundtrip failed for {schema_name} (save_merged={save_merged})")

        # Compare all formats to each other
        self.assertEqual(schema_from_xml, schema_from_mediawiki,
                        f"XML vs MediaWiki mismatch for {schema_name} (save_merged={save_merged})")
        self.assertEqual(schema_from_xml, schema_from_tsv,
                        f"XML vs TSV mismatch for {schema_name} (save_merged={save_merged})")
        self.assertEqual(schema_from_xml, schema_from_json,
                        f"XML vs JSON mismatch for {schema_name} (save_merged={save_merged})")
    ...
    def test_format_compatibility_standard(self):
        ...
        for i, format1 in enumerate(format_names):
            for format2 in format_names[i+1:]:
                self.assertEqual(schemas[format1], schemas[format2],
    ...
    def test_json_specific_features(self):
        ...
        if "Language-item-property" in schema.tags:
            ...
            self.assertEqual(original_tag.attributes, reloaded_tag.attributes,
                           "Tag attributes don't match after JSON roundtrip")
