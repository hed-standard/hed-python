...
class TestCLIParameterParity(unittest.TestCase):
    ...
    def _get_click_options(self, command):
        ...
        for param in command.params:
            if isinstance(param, click.core.Argument):
                ...
                if param.is_flag:
                    flags.append((param.name, [opt for opt in param.opts if len(opt) == 2], 
    ...
    def test_validate_bids_parameters(self):
        ...
        self.assertEqual(len(cli_opts['positional']), len(original_opts['positional']),
        ...
        for orig_dest, cli_dest in dest_mappings.items():
            if orig_dest in original_dests:
                self.assertIn(cli_dest, cli_dests,
        ...
        for flag in required_flags:
            if flag in original_flags:
                self.assertIn(flag, cli_flags,
                            f"Flag '{flag}' from original parser not found in CLI")
    ...
    def test_remodel_run_parameters(self):
        ...
        self.assertEqual(len(cli_opts['positional']), 2, 
                        f"Should have 2 positional arguments, got {len(cli_opts['positional'])}")
        ...
        cli_dests = set(cli_opts['optional'].keys())
        required_params = ['backup_dir', 'backup_name', 'suffixes', 'individual_summaries', 
                          'json_sidecar', 'log_dir', 'hed_versions', 'save_formats', 
                          'task_names', 'work_dir', 'exclude_dirs']
        for param in required_params:
    ...
    def test_extract_sidecar_parameters(self):
        ...
        # Check key parameters
        cli_dests = set(cli_opts['optional'].keys())
        required_params = ['suffix', 'value_columns', 'skip_columns', 'log_level', 
                          'log_file', 'output_file', 'exclude_dirs']
        for param in required_params:
    ...
    def test_schema_validate_parameters(self):
        ...
        self.assertIn('schema_path', cli_opts['positional'], 
                     "schema_path positional argument not found")
    ...
    def test_schema_add_ids_parameters(self):
        ...
        self.assertEqual(len(cli_opts['positional']), 3,
                        f"Should have 3 positional arguments, got {len(cli_opts['positional'])}")
    ...
    def test_all_legacy_commands_have_cli_equivalents(self):
        ...
        for legacy, cli_path in legacy_to_cli.items():
            ...
            if len(parts) == 1:
                # Top-level command
                self.assertIn(parts[0], cli.commands, 
                            f"CLI command '{parts[0]}' not found for legacy '{legacy}'")
            else:
                # Subcommand
                group = cli.commands.get(parts[0])
                self.assertIsNotNone(group, f"CLI group '{parts[0]}' not found for legacy '{legacy}'")
                if hasattr(group, 'commands'):
                    self.assertIn(parts[1], group.commands,
