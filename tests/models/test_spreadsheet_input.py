...
class Test(unittest.TestCase):
    ...
    def test_to_excel_should_work(self):
        spreadsheet = SpreadsheetInput(
            file=self.default_test_file_name,
            file_type=".xlsx",
            tag_columns=[3],
            has_column_names=True,
            column_prefix_dictionary={1: "Label/", 2: "Description/"},
    ...
    def test_no_column_header_and_convert(self):
        ...
        events_path_long = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../data/model_tests/no_column_header_long.tsv"
        )
    ...
    def test_convert_short_long_with_definitions(self):
        ...
        events_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../data/model_tests/no_column_header_definition.tsv"
        )
        hed_input = SpreadsheetInput(events_path, has_column_names=False, tag_columns=[0, 1])
        hed_input.convert_to_long(self.hed_schema)

        events_path_long = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../data/model_tests/no_column_header_definition_long.tsv"
        )
        hed_input_long = SpreadsheetInput(events_path_long, has_column_names=False, tag_columns=[0, 1])
        self.assertTrue(hed_input._dataframe.equals(hed_input_long._dataframe))
    ...
    def test_ignoring_na_column(self):
        events_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../data/model_tests/na_tag_column.tsv")
        hed_input = SpreadsheetInput(events_path, has_column_names=False, tag_columns=[0, 1])
    ...
    def test_to_excel_workbook(self):
        excel_book = SpreadsheetInput(self.default_test_file_name, worksheet_name="LKT 8HED3", tag_columns=["HED tags"])
    ...
    def test_to_excel_workbook_no_col_names(self):
        excel_book = SpreadsheetInput(
            self.default_test_file_name, worksheet_name="LKT 8HED3", tag_columns=[4], has_column_names=False
        )
