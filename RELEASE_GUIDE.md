...
1. **Choose a tag:** Type a new tag: `<version>`
   ...
   - Do NOT use a prefix (e.g., use `<version>`, not `v<version>`)
...
1. Go to: [https://zenodo.org/search?q=hed-python](https://zenodo.org/search?q=hed-python) (or the latest Zenodo record for hed-python)
...
- `dist/hedtools-<version>-py3-none-any.whl` (wheel distribution)
- `dist/hedtools-<version>.tar.gz` (source distribution)
...
python -m zipfile -l dist/hedtools-<version>-py3-none-any.whl

# Check the source distribution (requires tar in PATH)
tar -tzf dist/hedtools-<version>.tar.gz
```
...
```bash
# List contents of the wheel
unzip -l dist/hedtools-<version>-py3-none-any.whl
...
pip install dist/hedtools-<version>-py3-none-any.whl
```
...
# Test a basic import
python -c "from hed import HedString; print('Success!')"
```
...
Expected output: `Checking dist/hedtools-<version>.tar.gz: PASSED` (and same for .whl)
...
```bash
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ hedtools==<version>
```
...
Go to: https://github.com/hed-standard/hed-python/releases/tag/<version>

#### 8.2 Edit release and attach files

- Click **Edit release**
- Drag and drop or upload:
  - `dist/hedtools-<version>-py3-none-any.whl`
  - `dist/hedtools-<version>.tar.gz`
...
- **MINOR:** New features, new functions/classes, enhancements
...
git commit -m "Update CHANGELOG for v<version>"   # Commit
