...
def schema_error_hed_duplicate_node(tag, duplicate_tag_list, section):
    tag_join_delimiter = "\n\t"
    return (
        f"Duplicate term '{str(tag)}' used {len(duplicate_tag_list)} places in '{section}' section schema as:"
...
def schema_error_hed_duplicate_from_library(tag, duplicate_tag_list, section):
    tag_join_delimiter = "\n\t"
    return (
        f"Duplicate term '{str(tag)}' found in library and standard schemas in '{section}' section schema as:"
        + f"{tag_join_delimiter}{tag_join_delimiter.join(duplicate_tag_list)}"
    )
...
def schema_error_unknown_attribute(attribute_name, source_tag):
    return (
        f"Attribute '{attribute_name}' used by '{source_tag}' "
...
def schema_error_SCHEMA_PRERELEASE_VERSION_USED(current_version, known_versions):
    return (
        f"Schema version {current_version} used, which is prerelease or unofficial. "
...
def schema_warning_non_placeholder_class(tag_name, invalid_attribute_name):
    return (
        "Only placeholder nodes('#') can have a unit class, value class, or takes value."
        + f"Found {invalid_attribute_name} on {tag_name}"
    )


@hed_error(SchemaAttributeErrors.SCHEMA_DEPRECATED_INVALID, actual_code=SchemaAttributeErrors.SCHEMA_DEPRECATION_ERROR)
...
@hed_error(SchemaAttributeErrors.SCHEMA_CHILD_OF_DEPRECATED, actual_code=SchemaAttributeErrors.SCHEMA_DEPRECATION_ERROR)
...
@hed_error(
    SchemaAttributeErrors.SCHEMA_DEFAULT_UNITS_INVALID, actual_code=SchemaAttributeErrors.SCHEMA_ATTRIBUTE_VALUE_INVALID
)
