...
class BidsDataset:
    """ CRepresents the metadata for a BIDS dataset primarily focused on HED evaluation."""
    ...
    def __init__(self, root_path, schema=None, tabular_types=None):
        ...
        Args:
            ...
            schema (HedSchema or HedSchemaGroup):  An optional HedSchema or Hed that overrides the one specified in dataset.
        ...
        self.tabular_files = {"participants": BidsFileGroup(root_path, suffix="participants", type="tabular")}
        if not tabular_types:
            self.tabular_files["events"] = BidsFileGroup(root_path, suffix="events", type="tabular")
        else:
            for suffix in tabular_types:
                self.tabular_files[type] = BidsFileGroup(root_path, suffix=suffix, type="tabular")

    def get_tabular_group(self, type="events"):
        if type in self.tabular_files:
            return self.tabular_files[type]
        else:
            return None
