...
class VariableManager:
    ...
    def get_variable_factors(self, variables=None, as_one_hot=True):
        if variables is None:
            variables = self.variables
        df_list = [0]*len(variables)
        for index, variable in enumerate(variables):
            var_sum = self._variable_map[variable]
            df_list[index] = var_sum.get_factors(as_one_hot=as_one_hot)
...
if __name__ == '__main__':
    ...
    df.to_csv("D:/wh_conditions.csv", sep='\t', index=False)
    with open('d:/wh_summary.json', 'w') as f:
        json.dump(summary, f, indent=4)
    print("to here")

    df = var_manager.get_variable_factors(as_one_hot=False)
