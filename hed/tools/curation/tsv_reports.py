...
def report_tsv_diffs(tsv_dict1, tsv_dict2, logger):
    ...
    if len(tsv_dict1.key_list) != len(tsv_dict2.key_list):
        logger.add("overall",
                   f"---ERROR--- {tsv_dict1.name} has {len(tsv_dict1.file_list)} files and"
                   f" {tsv_dict2.name} has {len(tsv_dict2.file_list)} files", also_print=True)

    # Compare keys from the two dictionaries to make sure they have the same keys
    key_diff = tsv_dict1.key_diffs(tsv_dict2)
    if key_diff:
        logger.add("overall", f"---ERROR--- file key differences {str(key_diff)}", also_print=True)
    ...
    for key, file, rowcount, columns in tsv_dict2.iter_tsv_info():
        logger.add(key, f"{tsv_dict2.name}: [{rowcount} events] {str(columns)}")
    ...
    if count_diffs:
        print(f"The number of {tsv_dict1.name} events and {tsv_dict2.name} events differ for the following files:")
        for item in count_diffs:
            logger.add(key, item[0], f"---ERROR--- The {tsv_dict1.name} file has {item[1]} rows and"
                       f"the {tsv_dict2.name} event file has {item[2]} rows", also_print=True)
    else:
        logger.add(key, f"The {tsv_dict1.name} event files and the {tsv_dict2.name} files have the same number of rows",
                   also_print=True)
