...
class DefinitionDict:
    ...
    def _add_definition(self, def_tag, def_value):
        if def_tag in self.defs and def_value == self.defs[def_tag]:
            ...
            self._issues += ErrorHandler.format_error_from_context(
                DefinitionErrors.DUPLICATE_DEFINITION,
                error_context=error_context,
                def_name=def_tag,
                actual_error=DefinitionErrors.DUPLICATE_DEFINITION,
            )
        else:
    ...
    def check_for_definitions(self, hed_string_obj, error_handler=None) -> list[dict]:
        ...
        for definition_tag, group in hed_string_obj.find_top_level_tags(anchor_tags={DefTagNames.DEFINITION_KEY}):
            ...
            self.defs[def_tag_name.casefold()] = DefinitionEntry(
                name=def_tag_name, contents=group_tag, takes_value=def_takes_value, source_context=context
            )
    ...
    def _validate_name_and_context(self, def_tag_name, error_handler):
        ...
        if def_tag_name.casefold() in self.defs:
            new_def_issues += ErrorHandler.format_error_with_context(
                error_handler, DefinitionErrors.DUPLICATE_DEFINITION, def_name=def_tag_name
            )
    ...
    def _validate_placeholders(def_tag_name, group, def_takes_value, error_handler):
        ...
        if tags_with_issues:
            new_issues += ErrorHandler.format_error_with_context(
                error_handler,
    ...
    def _find_group(definition_tag, group, error_handler):
        ...
        if len(groups) > 1:
            issues += ErrorHandler.format_error_with_context(
                error_handler, DefinitionErrors.WRONG_NUMBER_GROUPS, def_name=definition_tag.extension, tag_list=groups
            )
        elif len(groups) == 0 and "#" in definition_tag.extension:
            issues += ErrorHandler.format_error_with_context(
                error_handler, DefinitionErrors.NO_DEFINITION_CONTENTS, def_name=definition_tag.extension
            )
        if len(group.tags()) != 1:
            issues += ErrorHandler.format_error_with_context(
                error_handler,
