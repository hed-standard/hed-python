...
def check_compliance(hed_schema, check_for_warnings=True, name=None, error_handler=None) -> list:
    ...
    Returns:
        list: A list of all warnings and errors found in the file. Each issue is a dictionary.
            The returned list has an additional attribute ``compliance_summary`` (ComplianceSummary)
            that provides a structured report of what was checked and the results.
    ...
    issues_list = _IssuesListWithSummary(issues_list, validator.summary)
    return issues_list
...
class SchemaValidator:
    ...
    def check_if_prerelease_version(self):
        self.summary.start_check("prerelease_version", "Check if schema version is newer than all known released versions.")
        issues = []
    ...
    def check_prologue_epilogue(self):
        self.summary.start_check("prologue_epilogue", "Validate characters in prologue and epilogue text (8.3+ only).")
        issues = []
        if self._new_character_validation:
            ...
            self.summary.add_sub_check("skipped (pre-8.3 schema)")
        self.error_handler.add_context_and_filter(issues)
        self.summary.record_issues(len(issues))
        return issues
    ...
    def check_attributes(self):
        ...
        self.summary.start_check("attributes", "Validate all attribute values on every entry in every section.")
        self.summary.add_sub_check("unknown attribute detection")
        self.summary.add_sub_check("deprecated attribute usage")
        if self._new_character_validation:
            self.summary.add_sub_check("hedId validation")
            self.summary.add_sub_check("range-based attribute value validation")
        issues_list = []
        for section_key in HedSectionKey:
            ...
            for tag_entry in self.hed_schema[section_key].values():
                ...
                new_issues = self._check_tag_entry_attributes(tag_entry)
                issues_list += new_issues
                self.error_handler.pop_error_context()
            self.summary.record_section(section_key, entry_count)
            self.error_handler.pop_error_context()
        self.summary.record_issues(len(issues_list))
        # Record which specific attribute validators were used
        validators_dict = self.attribute_validators if self._new_character_validation else self.attribute_validators_old
        for attr_name, validators in validators_dict.items():
            for v in validators:
                v_name = getattr(v, "__name__", None) or getattr(v, "func", lambda: None).__name__
                self.summary.add_sub_check(f"{attr_name}: {v_name}")
        return issues_list
    ...
    def check_duplicate_names(self):
        ...
        self.summary.start_check("duplicate_names", "Check for duplicate entry names within or across library merges.")
        issues_list = []
        for section_key in HedSectionKey:
            entry_count = len(self.hed_schema[section_key])
            len(self.hed_schema[section_key].duplicate_names)
            self.summary.record_section(section_key, entry_count)
    ...
    def check_invalid_chars(self):
        ...
        self.summary.start_check("invalid_characters", "Validate characters in entry names and descriptions.")
        issues_list = []
        section_validators = {
        ...
        if self._new_character_validation:
            ...
            self.summary.add_sub_check("tag name capitalization (8.3+ character classes)")
            self.summary.add_sub_check("term name character validation (8.3+ character classes)")
            self.summary.add_sub_check("description character validation (8.3+ character classes)")
        else:
            self.summary.add_sub_check("tag name capitalization (legacy)")
            self.summary.add_sub_check("term name: block curly braces (legacy)")
            self.summary.add_sub_check("description character validation (legacy)")
