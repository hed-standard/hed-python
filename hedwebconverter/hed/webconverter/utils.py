...
from hed.webconverter.constants.form import python_form_constants, validation_arg_constants, js_form_constants, \
    html_form_constants
...
def _get_uploaded_file_paths_from_forms(form_request_object):
    ...
    """
    hed_file_path = ''
    result1 = hed_present_in_form(form_request_object)
    result2 = _file_has_valid_extension(
    ...
    if url_present_in_form(form_request_object):
...
def find_hed_version_in_file(form_request_object):
    ...
    try:
        if hed_file_present_in_form(form_request_object):
            ...
            hed_info[js_form_constants.HED_VERSION] = "Uploaded!"
...
def run_conversion(form_request_object):
    ...
    try:
        ...
        result_dict = xml2wiki.convert_hed_xml_2_wiki(None, validation_input_arguments[validation_arg_constants.HED_XML_PATH])
        ...
        validation_status[js_form_constants.DOWNLOAD_FILE] = _save_validation_issues_to_file_in_upload_folder(
            result_dict[converter_constants.HED_WIKI_LOCATION_KEY],
            "output.mediawiki")
        validation_status[js_form_constants.ISSUE_COUNT] = 5
        validation_status[js_form_constants.ERROR_COUNT] = 0
        validation_status[js_form_constants.WARNING_COUNT] = 5
    except:
...
def get_all_hed_versions():
    ...
    for _, _, hed_files in os.walk(HED_DIRECTORY):
...
def find_major_hed_versions():
    ...
    hed_info = {}
    try:
        hed_info[js_form_constants.HED_MAJOR_VERSIONS] = get_all_hed_versions()
    except:
        hed_info[error_constants.ERROR_KEY] = traceback.format_exc()
    return hed_info
...
def _save_validation_issues_to_file_in_upload_folder(result_mediawiki_name, final_output_name=''):
    """Saves the validation issues found to a other in the upload folder.

    """
    validation_issues_filename = _generate_spreadsheet_validation_filename(result_mediawiki_name, final_output_name)
    validation_issues_file_path = os.path.join(current_app.config[UPLOAD_DIRECTORY_KEY], validation_issues_filename)
    copyfile(result_mediawiki_name, validation_issues_file_path)
...
def _generate_spreadsheet_validation_filename(spreadsheet_filename, worksheet_name=''):
    ...
    if worksheet_name:
        return file_extension_constants.VALIDATION_OUTPUT_FILE_PREFIX + \
               ...
               secure_filename(worksheet_name)
...
def _generate_input_arguments_from_validation_form(form_request_object,
                                                   hed_file_path):
...
def get_path_from_hed_version(hed_version):
    ...
    """
    return os.path.join(HED_DIRECTORY,
                        HED_XML_PREFIX + hed_version + HED_XML_EXTENSION)
...
def _get_hed_path_from_validation_form(form_request_object, hed_file_path):
    ...
    if _hed_version_in_form(form_request_object) and \
            ((form_request_object.form[
                 html_form_constants.HED_VERSION] != spreadsheet_constants.OTHER_HED_VERSION_OPTION and form_request_object.form[
                 html_form_constants.HED_VERSION] != spreadsheet_constants.URL_HED_VERSION_OPTION) or not hed_file_path):
        return get_path_from_hed_version(form_request_object.form['hed-version'])
    return hed_file_path
...
def url_present_in_form(validation_form_request_object):
    """Checks to see if a HED XML URL is present in a request object from validation form.
    ...
    """
    return js_form_constants.HED_URL in validation_form_request_object.values
